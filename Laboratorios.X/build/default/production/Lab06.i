...
reinicioTMR0 macro
    ...
    bcf ((INTCON) and 07Fh), 2 ;Limpiamos la bandera de Overflow del Timer 0
    endm
...
reinicioTMR1 macro
    ...
    bcf ((PIR1) and 07Fh), 0 ;Limpiamos la bandera de Overflow del Timer 1
    endm
...
reinicioTMR2 macro
    ...
    banksel PORTA
    bcf ((PIR1) and 07Fh), 1 ;Limpiamos la bandera de Overflow del Timer 2
    endm
...
isr:
    btfsc ((INTCON) and 07Fh), 2 ;Verificar si la bandera de overflow del TMR0 esta activa
    call intTMR0 ;Si esta activa llamar la subrutina

    btfsc ((PIR1) and 07Fh), 0 ;Verificar si la bandera de overflow del TMR1 esta activa
    call intTMR1 ;Si esta activa llamar la subrutina

    btfsc ((PIR1) and 07Fh), 1 ;Verificar si la bandera de overflow del TMR2 esta activa
...
intTMR0:
    ...
    bcf PORTD, 0 ;Limpiamos los pines ((PORTD) and 07Fh), 0, ((PORTD) and 07Fh), 1
    bcf PORTD, 1
...
display1:
   ...
   bsf PORTD, 0 ;Activamos ((PORTD) and 07Fh), 0
...
display2:
   ...
   movwf PORTC ;Lo movemos al PORTC
   bsf PORTD, 1 ;Activamos ((PORTD) and 07Fh), 1
   bcf flags, 0
   return
...
config_int:
    ...
    bsf ((INTCON) and 07Fh), 7 ;Habilitar las interupciones globales
    bsf ((INTCON) and 07Fh), 6

    bsf ((INTCON) and 07Fh), 5 ;Habilitamos la interupcion por overflow del TMR0
    bcf ((INTCON) and 07Fh), 2 ;Limpiar la bandera de overflow del TMR0

    banksel TRISA
    bsf ((PIE1) and 07Fh), 0 ;Habilitamos la interupcion por overflow del TMR1
    bsf ((PIE1) and 07Fh), 1 ;Habilitamos la interupcion por overflow del TMR2

    banksel PORTA
    bcf ((PIR1) and 07Fh), 0 ;Limpiar la bandera de overflow del TMR1
    bcf ((PIR1) and 07Fh), 1 ;Limpiar la bandera de overflow del TMR2
    return

config_TMR0:
    banksel TRISA ;Nos movemos al banco 1
    bcf ((OPTION_REG) and 07Fh), 5 ;TMR0 usa el reloj interno
    bcf ((OPTION_REG) and 07Fh), 3 ;Prescaler es utilizado por el TMR0
    bsf ((OPTION_REG) and 07Fh), 2
    bsf ((OPTION_REG) and 07Fh), 1
    bsf ((OPTION_REG) and 07Fh), 0 ;Prescaler PS (111) = 1:256
...
config_TMR1:
    banksel PORTA
    bsf ((T1CON) and 07Fh), 0 ;TMR1 enable
    bcf ((T1CON) and 07Fh), 1 ;TMR1 clock internal
    bsf ((T1CON) and 07Fh), 4 ;Prescaler (ss) 1:8
...
display_setup:
    ...
    bsf PORTD, 2 ;Encendemos el led en ((PORTD) and 07Fh), 2
    return
...
turnoff:
    ...
    bcf PORTD, 2 ;Apagamos el led en ((PORTD) and 07Fh), 2
    return
