...
isr:
    btfsc ((INTCON) and 07Fh), 0 ;Verificar si la bandera de int on change en B esta activa
...
intB:
    ...
    btfsc PORTB, 0 ;Verificar si el boton en ((PORTB) and 07Fh), 0 se esta presionando
    incf PORTA ;Si se presiona incrementar el PORTA

    btfsc PORTB, 1 ;Verificar si el boton en ((PORTB) and 07Fh), 1 se esta presionando
    decf PORTA ;Si se presiona decrementar el PORTA

    bcf ((INTCON) and 07Fh), 0 ;Limpiar la bandera de int on change B
    return

intTMR0:
    reinicioTMR0 ;Reiniciamos el TMR0
    bcf PORTB, 2 ;Limpiamos los pines ((PORTB) and 07Fh), 2, ((PORTB) and 07Fh), 3, ((PORTB) and 07Fh), 4, ((PORTB) and 07Fh), 5, ((PORTB) and 07Fh), 6, ((PORTB) and 07Fh), 7
    bcf PORTB, 3
    bcf PORTB, 4
    bcf PORTB, 5
    bcf PORTB, 6
...
display1:
   ...
   bsf PORTB, 2 ;Activamos ((PORTB) and 07Fh), 2
...
display2:
   ...
   movwf PORTC ;Lo movemos al PORTC
   bsf PORTB, 3 ;Activamos ((PORTB) and 07Fh), 3
...
display3:
   ...
   bsf PORTB, 4 ;Activamos ((PORTB) and 07Fh), 4
...
display4:
   ...
   movwf PORTD ;Lo movemos al PORTD
   bsf PORTB, 5 ;Activamos ((PORTB) and 07Fh), 5
...
display5:
   ...
   movwf PORTD ;Lo movemos al PORTD
   bsf PORTB, 6 ;Activamos ((PORTB) and 07Fh), 6
...
display6:
   ...
   movwf PORTD ;Lo movemos al PORTD
   bsf PORTB, 7 ;Activamos ((PORTB) and 07Fh), 7
...
config_CLK:
    ...
    bsf ((OSCCON) and 07Fh), 6 ;Reloj de 8 MHz IRCF = 111
...
config_int:
    ...
    bsf ((INTCON) and 07Fh), 3 ;Habilitar las interupciones on change en B
    bcf ((INTCON) and 07Fh), 0 ;Limpiar la bandera de int on change B
...
config_intchB:
    ...
    bsf IOCB, 0 ;Habilitar int on change en ((PORTB) and 07Fh), 0
    bsf IOCB, 1 ;Habilitar int on change en ((PORTB) and 07Fh), 1
